# APWifipico.py — AP + HTTP estático index.html + /on /off /state (POST JSON) y /adc
...
def serve(index_bytes):
    ...
    while True:
        try:
            ...
            # --- LECTURA ADC0 ---
            if path == "/adc":
                valor = adc0.read_u16()  # valor entre 0 y 65535
                body = '{{"adc":{}}}'.format(valor)
                http_send(conn, "200 OK", "application/json; charset=utf-8", body,
                          {"Cache-Control":"no-cache"})
                conn.close(); gc.collect(); continue

            # Raíz u otra ruta -> index.html
